...
cdef class BoxShape(ConvexShape):
    ...
    with length specified by half extents, in local shape coordinates.
...
cdef class CylinderShape(ConvexShape):
    ...
    """
    def __cinit__(self, Vector3 halfExtents):
...
cdef class StaticPlaneShape(CollisionShape):
    """
    A static (unmovable) plane.
    This class is a wrapper around btStaticPlaneShape.
    """
    def __cinit__(self, Vector3 normal, btScalar constant):
...
# FIXME http://www.bulletphysics.com/Bullet/BulletFull/classbtBvhTriangleMeshShape.html
# says that this inherits from ConcaveShape!
...
cdef class OverlappingPairCache:
    ...
    def cleanProxyFromPairs(self, BroadphaseProxy proxy, CollisionDispatcher dispatcher):
        # TODO should take a Dispatcher, but we don't wrap that yet (we directly wrap the subclass CollisionDispatcher).
        self.thisptr.cleanProxyFromPairs(proxy.thisptr, dispatcher.thisptr)
...
cdef class RigidBody(CollisionObject):
    ...
    def __init__(self,
        ...
        info = new btRigidBodyConstructionInfo(mass, 
            self.motion.thisptr, shape.thisptr, inertia)
...
cdef class AxisSweep3(BroadphaseInterface):
    ...
    def getOverlappingPairCache(self):
        cdef OverlappingPairCache cache = OverlappingPairCache()
        cache.thisptr = (<btAxisSweep3*>self.thisptr).getOverlappingPairCache()
        return cache
